# This file is a modification of functions in LTRCforests package
...
predictProb.ltrccif <- function(object, newdata = NULL, newdata.id, OOB = FALSE,
  ...
  if (is.null(newdata) || OOB){
    # Modify the original function in LTRCforests
    # first column: Surv(tleft,tright,event), second column: (id)
    newdata <- cbind.data.frame(as.matrix(object$data[,1]),object$data[,ncol(object$data)])
